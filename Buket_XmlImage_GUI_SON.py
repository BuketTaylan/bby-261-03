...
class ResimGosterici:
    def __init__(self, ana_pencere, xml_dosya_yolu):
        ...
        self.ana_pencere.title("Kitap Resim Galerisi")
    ...
    def _alt_satira_indir(self, metin, karakter_sayisi=50):
        if len(metin) > karakter_sayisi:
            # Belirli bir karakter sayısında bir alt satıra geç
    ...
    def _listele_penceresi(self):
        ...
        text_widget = Text(listele_pencere, wrap='none', width=80, height=20, font=('Arial', 10))
    ...
    def _arayuz_olustur(self):
        ...
        self.bilgi_text = Text(self.ana_pencere, wrap='none', width=50, height=10, font=('Arial', 10))
        self.bilgi_text.pack()

        onceki_button = Button(self.ana_pencere, text="Önceki Kitap", command=self._onceki_kitabi_goster)
        onceki_button.pack(side="left")

        sonraki_button = Button(self.ana_pencere, text="Sonraki Kitap", command=self._sonraki_kitabi_goster)
...
if __name__ == "__main__":
    xml_dosya_yolu = "xml_files/kitaplar.xml"
    root = Tk()
    uygulama = ResimGosterici(root, xml_dosya_yolu)
    root.mainloop()
